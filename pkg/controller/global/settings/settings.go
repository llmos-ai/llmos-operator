...
func Register(ctx context.Context, scaled *config.Scaled) error {
	setting := scaled.MgmtFactory.Management().V1().Setting()
	sp := &handler{
		context:      ctx,
		settings:     setting,
...
func (s *handler) Set(name, value string) error {
...
func (s *handler) SetIfUnset(name, value string) error {
	obj, err := s.settings.Get(name, metav1.GetOptions{})
	if err != nil {
		return err
	}
