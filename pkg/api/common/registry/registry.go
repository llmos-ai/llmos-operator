...
func (h BaseHandler) upload(rw http.ResponseWriter, req *http.Request, namespace, name string) error {
	...
	for i := range files {
		...
		go func(i int) {
			defer wg.Done()
			errors <- h.uploadOneFile(rw, b, files[i], path.Join(rootPath, input.TargetDirectory, relativePath))
		}(i)
	}
...
func (h BaseHandler) uploadOneFile(rw http.ResponseWriter, b backend.Backend, fileHeader *multipart.FileHeader, targetPath string) error {
	// Open the file
...
func reportProgress(ctx context.Context, rw http.ResponseWriter, processChan chan int64, totalSize int64, destPath string) {
	for {
		select {
