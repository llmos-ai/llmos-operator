...
// this is referred to the condition package in the wrangler library
import (
	"errors"
	"os"
	"reflect"
...
func (c Cond) SetError(obj interface{}, reason string, err error) {
	if err == nil || errors.Is(err, errors.New("ignore handler error")) {
		c.True(obj)
		c.Message(obj, "")
		c.Reason(obj, reason)
		return
...
func findCond(obj interface{}, val reflect.Value, name string) *reflect.Value {
	defer func() {
		if recover() != nil {
			logrus.Errorf("failed to find .Status.Conditions field on %v", reflect.TypeOf(obj))
			os.Exit(1)
		}
	}()
