...
// RegisterManagedAddonStatusHandler configures a ManagedAddonController to execute a ManagedAddonStatusHandler for every events observed.
...
func RegisterManagedAddonStatusHandler(ctx context.Context, controller ManagedAddonController, condition condition.Cond, name string, handler ManagedAddonStatusHandler) {
	statusHandler := &managedAddonStatusHandler{
		client:    controller,
		condition: condition,
		handler:   handler,
	}
...
// RegisterManagedAddonGeneratingHandler configures a ManagedAddonController to execute a ManagedAddonGeneratingHandler for every events observed, passing the returned objects to the provided apply.Apply.
// If a non-empty condition is provided, it will be updated in the status conditions for every handler execution
func RegisterManagedAddonGeneratingHandler(ctx context.Context, controller ManagedAddonController, apply apply.Apply,
	...
	statusHandler := &managedAddonGeneratingHandler{
		ManagedAddonGeneratingHandler: handler,
		apply:                         apply,
		name:                          name,
	...
	RegisterManagedAddonStatusHandler(ctx, controller, condition, name, statusHandler.Handle)
}

type managedAddonStatusHandler struct {
	client    ManagedAddonClient
	condition condition.Cond
	handler   ManagedAddonStatusHandler
}

// sync is executed on every resource addition or modification. Executes the configured handlers and sends the updated status to the Kubernetes API
func (a *managedAddonStatusHandler) sync(key string, obj *v1.ManagedAddon) (*v1.ManagedAddon, error) {
	if obj == nil {
		return obj, nil
	}
...
type managedAddonGeneratingHandler struct {
	ManagedAddonGeneratingHandler
	apply apply.Apply
...
func (a *managedAddonGeneratingHandler) Remove(key string, obj *v1.ManagedAddon) (*v1.ManagedAddon, error) {
	if obj != nil {
		return obj, nil
	}
...
// Handle executes the configured ManagedAddonGeneratingHandler and pass the resulting objects to apply.Apply, finally returning the new status of the resource
func (a *managedAddonGeneratingHandler) Handle(obj *v1.ManagedAddon, status v1.ManagedAddonStatus) (v1.ManagedAddonStatus, error) {
	if !obj.DeletionTimestamp.IsZero() {
		return status, nil
	}

	objs, newStatus, err := a.ManagedAddonGeneratingHandler(obj, status)
	if err != nil {
...
func (a *managedAddonGeneratingHandler) isNewResourceVersion(obj *v1.ManagedAddon) bool {
	if !a.opts.UniqueApplyForResourceVersion {
		return true
	}
...
// Only used if UniqueApplyForResourceVersion is set in generic.GeneratingHandlerOptions
func (a *managedAddonGeneratingHandler) storeResourceVersion(obj *v1.ManagedAddon) {
	if !a.opts.UniqueApplyForResourceVersion {
		return
	}
