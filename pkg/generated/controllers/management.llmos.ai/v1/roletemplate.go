...
// RegisterRoleTemplateStatusHandler configures a RoleTemplateController to execute a RoleTemplateStatusHandler for every events observed.
...
// RegisterRoleTemplateGeneratingHandler configures a RoleTemplateController to execute a RoleTemplateGeneratingHandler for every events observed, passing the returned objects to the provided apply.Apply.
// If a non-empty condition is provided, it will be updated in the status conditions for every handler execution
...
// Handle executes the configured RoleTemplateGeneratingHandler and pass the resulting objects to apply.Apply, finally returning the new status of the resource
...
func (a *roleTemplateGeneratingHandler) isNewResourceVersion(obj *v1.RoleTemplate) bool {
	if !a.opts.UniqueApplyForResourceVersion {
		return true
	}
...
// Only used if UniqueApplyForResourceVersion is set in generic.GeneratingHandlerOptions
func (a *roleTemplateGeneratingHandler) storeResourceVersion(obj *v1.RoleTemplate) {
	if !a.opts.UniqueApplyForResourceVersion {
		return
	}
