...
// Handle executes the configured RayServiceGeneratingHandler and pass the resulting objects to apply.Apply, finally returning the new status of the resource
...
func (a *rayServiceGeneratingHandler) isNewResourceVersion(obj *v1.RayService) bool {
	if !a.opts.UniqueApplyForResourceVersion {
		return true
	}
...
// Only used if UniqueApplyForResourceVersion is set in generic.GeneratingHandlerOptions
func (a *rayServiceGeneratingHandler) storeResourceVersion(obj *v1.RayService) {
	if !a.opts.UniqueApplyForResourceVersion {
		return
	}
