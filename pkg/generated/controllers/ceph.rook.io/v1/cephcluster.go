...
// Handle executes the configured CephClusterGeneratingHandler and pass the resulting objects to apply.Apply, finally returning the new status of the resource
...
func (a *cephClusterGeneratingHandler) isNewResourceVersion(obj *v1.CephCluster) bool {
	if !a.opts.UniqueApplyForResourceVersion {
		return true
	}
...
// Only used if UniqueApplyForResourceVersion is set in generic.GeneratingHandlerOptions
func (a *cephClusterGeneratingHandler) storeResourceVersion(obj *v1.CephCluster) {
	if !a.opts.UniqueApplyForResourceVersion {
		return
	}
