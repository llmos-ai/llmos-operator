...
// RegisterModelServiceStatusHandler configures a ModelServiceController to execute a ModelServiceStatusHandler for every events observed.
...
func RegisterModelServiceStatusHandler(ctx context.Context, controller ModelServiceController, condition condition.Cond, name string, handler ModelServiceStatusHandler) {
	statusHandler := &modelServiceStatusHandler{
		client:    controller,
		condition: condition,
		handler:   handler,
	}
...
// RegisterModelServiceGeneratingHandler configures a ModelServiceController to execute a ModelServiceGeneratingHandler for every events observed, passing the returned objects to the provided apply.Apply.
// If a non-empty condition is provided, it will be updated in the status conditions for every handler execution
func RegisterModelServiceGeneratingHandler(ctx context.Context, controller ModelServiceController, apply apply.Apply,
	...
	statusHandler := &modelServiceGeneratingHandler{
		ModelServiceGeneratingHandler: handler,
		apply:                         apply,
		name:                          name,
	...
	RegisterModelServiceStatusHandler(ctx, controller, condition, name, statusHandler.Handle)
}

type modelServiceStatusHandler struct {
	client    ModelServiceClient
	condition condition.Cond
	handler   ModelServiceStatusHandler
}

// sync is executed on every resource addition or modification. Executes the configured handlers and sends the updated status to the Kubernetes API
func (a *modelServiceStatusHandler) sync(key string, obj *v1.ModelService) (*v1.ModelService, error) {
	if obj == nil {
		return obj, nil
	}
...
type modelServiceGeneratingHandler struct {
	ModelServiceGeneratingHandler
	apply apply.Apply
...
func (a *modelServiceGeneratingHandler) Remove(key string, obj *v1.ModelService) (*v1.ModelService, error) {
	if obj != nil {
		return obj, nil
	}
...
// Handle executes the configured ModelServiceGeneratingHandler and pass the resulting objects to apply.Apply, finally returning the new status of the resource
func (a *modelServiceGeneratingHandler) Handle(obj *v1.ModelService, status v1.ModelServiceStatus) (v1.ModelServiceStatus, error) {
	if !obj.DeletionTimestamp.IsZero() {
		return status, nil
	}

	objs, newStatus, err := a.ModelServiceGeneratingHandler(obj, status)
	if err != nil {
...
func (a *modelServiceGeneratingHandler) isNewResourceVersion(obj *v1.ModelService) bool {
	if !a.opts.UniqueApplyForResourceVersion {
		return true
	}
...
// Only used if UniqueApplyForResourceVersion is set in generic.GeneratingHandlerOptions
func (a *modelServiceGeneratingHandler) storeResourceVersion(obj *v1.ModelService) {
	if !a.opts.UniqueApplyForResourceVersion {
		return
	}
