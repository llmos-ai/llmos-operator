...
// RegisterModelFileStatusHandler configures a ModelFileController to execute a ModelFileStatusHandler for every events observed.
...
func RegisterModelFileStatusHandler(ctx context.Context, controller ModelFileController, condition condition.Cond, name string, handler ModelFileStatusHandler) {
	statusHandler := &modelFileStatusHandler{
		client:    controller,
		condition: condition,
		handler:   handler,
	}
...
// RegisterModelFileGeneratingHandler configures a ModelFileController to execute a ModelFileGeneratingHandler for every events observed, passing the returned objects to the provided apply.Apply.
// If a non-empty condition is provided, it will be updated in the status conditions for every handler execution
func RegisterModelFileGeneratingHandler(ctx context.Context, controller ModelFileController, apply apply.Apply,
	...
	statusHandler := &modelFileGeneratingHandler{
		ModelFileGeneratingHandler: handler,
		apply:                      apply,
		name:                       name,
	...
	RegisterModelFileStatusHandler(ctx, controller, condition, name, statusHandler.Handle)
}

type modelFileStatusHandler struct {
	client    ModelFileClient
	condition condition.Cond
	handler   ModelFileStatusHandler
}

// sync is executed on every resource addition or modification. Executes the configured handlers and sends the updated status to the Kubernetes API
func (a *modelFileStatusHandler) sync(key string, obj *v1.ModelFile) (*v1.ModelFile, error) {
	if obj == nil {
		return obj, nil
	}
...
type modelFileGeneratingHandler struct {
	ModelFileGeneratingHandler
	apply apply.Apply
...
func (a *modelFileGeneratingHandler) Remove(key string, obj *v1.ModelFile) (*v1.ModelFile, error) {
	if obj != nil {
		return obj, nil
	}
...
// Handle executes the configured ModelFileGeneratingHandler and pass the resulting objects to apply.Apply, finally returning the new status of the resource
func (a *modelFileGeneratingHandler) Handle(obj *v1.ModelFile, status v1.ModelFileStatus) (v1.ModelFileStatus, error) {
	if !obj.DeletionTimestamp.IsZero() {
		return status, nil
	}

	objs, newStatus, err := a.ModelFileGeneratingHandler(obj, status)
	if err != nil {
...
func (a *modelFileGeneratingHandler) isNewResourceVersion(obj *v1.ModelFile) bool {
	if !a.opts.UniqueApplyForResourceVersion {
		return true
	}
...
// Only used if UniqueApplyForResourceVersion is set in generic.GeneratingHandlerOptions
func (a *modelFileGeneratingHandler) storeResourceVersion(obj *v1.ModelFile) {
	if !a.opts.UniqueApplyForResourceVersion {
		return
	}
