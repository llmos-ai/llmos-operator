...
func RegisterClusterPolicyStatusHandler(ctx context.Context, controller ClusterPolicyController, condition condition.Cond, name string, handler ClusterPolicyStatusHandler) {
	statusHandler := &clusterPolicyStatusHandler{
		client:    controller,
		condition: condition,
		handler:   handler,
	}
...
func RegisterClusterPolicyGeneratingHandler(ctx context.Context, controller ClusterPolicyController, apply apply.Apply,
	condition condition.Cond, name string, handler ClusterPolicyGeneratingHandler, opts *generic.GeneratingHandlerOptions) {
	statusHandler := &clusterPolicyGeneratingHandler{
		ClusterPolicyGeneratingHandler: handler,
		apply:                          apply,
		name:                           name,
	...
	RegisterClusterPolicyStatusHandler(ctx, controller, condition, name, statusHandler.Handle)
}

type clusterPolicyStatusHandler struct {
	client    ClusterPolicyClient
	condition condition.Cond
	handler   ClusterPolicyStatusHandler
}

// sync is executed on every resource addition or modification. Executes the configured handlers and sends the updated status to the Kubernetes API
func (a *clusterPolicyStatusHandler) sync(key string, obj *v1.ClusterPolicy) (*v1.ClusterPolicy, error) {
	if obj == nil {
		return obj, nil
	}
...
type clusterPolicyGeneratingHandler struct {
	ClusterPolicyGeneratingHandler
	apply apply.Apply
...
func (a *clusterPolicyGeneratingHandler) Remove(key string, obj *v1.ClusterPolicy) (*v1.ClusterPolicy, error) {
	if obj != nil {
		return obj, nil
	}

	obj = &v1.ClusterPolicy{}
...
// Handle executes the configured ClusterPolicyGeneratingHandler and pass the resulting objects to apply.Apply, finally returning the new status of the resource
func (a *clusterPolicyGeneratingHandler) Handle(obj *v1.ClusterPolicy, status v1.ClusterPolicyStatus) (v1.ClusterPolicyStatus, error) {
	if !obj.DeletionTimestamp.IsZero() {
		return status, nil
	}

	objs, newStatus, err := a.ClusterPolicyGeneratingHandler(obj, status)
	if err != nil {
...
func (a *clusterPolicyGeneratingHandler) isNewResourceVersion(obj *v1.ClusterPolicy) bool {
	if !a.opts.UniqueApplyForResourceVersion {
		return true
	}
...
// Only used if UniqueApplyForResourceVersion is set in generic.GeneratingHandlerOptions
func (a *clusterPolicyGeneratingHandler) storeResourceVersion(obj *v1.ClusterPolicy) {
	if !a.opts.UniqueApplyForResourceVersion {
		return
	}
