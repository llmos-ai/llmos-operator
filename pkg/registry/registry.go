...
func (r *Manager) NewBackend(registry *mlv1.Registry) (backend.Backend, error) {
	...
	return s3.NewMinioClient(registry.Spec.S3Config.Endpoint, id, secret, registry.Spec.S3Config.Bucket, registry.Spec.S3Config.UseSSL)
}
...
func getAccessKey(secretCache ctlcorev1.SecretCache, accessCredentialSecretName string) (string, string, error) {
	...
	if !ok {
		return "", "", fmt.Errorf("secret %s does not contain %s key", accessCredentialSecretName, accessKeyID)
	}
...
func (r *Manager) NewBackendFromRegistry(registryName string) (backend.Backend, error) {
	registry, err := r.registryCache.Get(registryName)
	if err != nil {
		return nil, fmt.Errorf("get registry %s failed: %w", registryName, err)
	}

	return r.NewBackend(registry)
}
